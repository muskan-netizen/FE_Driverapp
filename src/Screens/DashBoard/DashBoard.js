  ...
  useFocusEffect(
    React.useCallback(() => {
      updateState({
        isLoading: true,
        options: [
          { label: getBundleId() == appIds.tdc  ? strings.TODAYTRIP : strings.TODAYSTASK, value: 0, testID: '1' },
          { label:getBundleId() ==  appIds.tdc  ? strings.ALLTRIP : strings.ALLTASKS, value: 1, testID: '2' },
        ],
      });
    }, []),
  );
          ...
          {(selectedOption ? allTasks?.length : todaysTasks?.length) ? (
            <FlatList
              ...
              ListEmptyComponent={() => (
                <ListEmptyComponent
                  isLoading={isLoading}
                  message={getBundleId() == appIds.tdc  ? strings.NOTRIP : strings.NOTASK}
